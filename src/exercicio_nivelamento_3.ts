...
function contarVgsParametro(palavra: string) {
  ...
  for (let i = 0; i < palavraUserInput.length; i++) {
    ...
    if (vogais.includes(char)) {
      ...
      alert(
        "ERRO: A palavra digitada não é válida! Por favor, tente novamente."
      );
      limparInput();
      return;
    }
  ...
  if (palavraUserInput) {
    alert(
      "A palavra " + palavraUserInput + " contém: " + quantidadeVgs + " vogais."
    );
    limparInput();
  } else {
...
function getBioById(id: number) {
  ...
  if (user) {
    const userItem = document.getElementById(`user${user.id}`) as HTMLLIElement;
    if (userItem) {
    ...
    alert(
      "ERRO: Nenhuma bio foi encontrada para o ID informado, ou nenhum ID foi informado."
    );
    limparInput();
  }
}
...
function getNameById(id: number) {
  ...
  if (user) {
    ...
    alert(
      "ERRO: Nenhum nome foi encontrado para o ID informado, ou nenhum ID foi informado."
    );
    limparInput();
  }
}
...
function deleteItemById(id: number) {
  ...
  if (userElement) {
    ...
    alert(
      "ERRO: Nenhum usuário encontrado com o ID informado, ou nenhum ID foi informado."
    );
    limparInput();
  }
}
